...
${\lambda}$ is the laser wavelength and  $\delta x_{sc}$ is the time-dependent displacement of the scattering surface which is added to the static path $x_{0}$~\cite{scatvirgo}, $L$ is the length of arms (4 kms), $\mathcal{F}$ is the Fourier transform of the scatterer motion \cite{vaj_scat}. 
\par
The build up of this phase shifted field in the arms by the factor ${\Gamma}$ pushes on the mirrors resulting in radiation pressure noise. This radiation component can be expressed as:

%The power fluctuations in the arm cavity push on the mirrors resulting in radiation pressure noise. This noise can be written as:

\begin{equation}
    h_{rad}(f) = A\frac{2{\Gamma P}}{Mc}\frac{2}{\Omega^2 - \omega^2} \mathcal{F}\left[\cos{\delta{\phi}}\right] \label{rad_eqn}
\end{equation}
${\Gamma}$ is cavity signal gain, $M = 40$ kgs is the mass of mirrors, $P$ is the power in the arms, $c$ is speed of light and ${\Omega}$ is the suspension eigenfrequency.
\begin{figure}[h]
    \centering
...
The total effective displacement power spectrum $S(f)$ can be obtained by adding the phase and radiation contributions:
...
where, $h_{ph}
(f)$ and $h_{rad}(f)$ are given by equation \ref{eq:1} and equation \ref{rad_eqn} respectively. $T_{end}$ is the ETM  transmission ($4e^{-6}$), L is the length of arms ($4$ kms), $f_{r}$ is the fraction of power reflected back from the scattering surface, and $\mathcal{F}$ is the Fourier transform of the scatterer motion \cite{vaj_scat}.
\begin{figure}[h]
\captionsetup[subfigure]{font=scriptsize,labelfont=scriptsize}
   \centering
    \begin{subfigure}[b]{0.45\textwidth}
