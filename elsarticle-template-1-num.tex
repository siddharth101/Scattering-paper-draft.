...
\end{equation}
${\Gamma}$ is cavity signal gain, $M = 40$ kgs is the mass of mirrors, $P$ is the power in the arms, $c$ is speed of light and ${\Omega}$ is the suspension eigenfrequency~\cite{Ottaway:12}.
\begin{figure}[h]
    \centering
...
$h_{ph}
(f)$ and $h_{rad}(f)$ are given by equation \ref{eq:1} and equation \ref{rad_eqn} respectively. $T_{end}$ is the ETM  transmission ($4e^{-6}$), $f_{r}$ is the fraction of power reflected back from the scattering surface.
\begin{figure}[h]
\captionsetup[subfigure]{font=scriptsize,labelfont=scriptsize}
   \centering
    \begin{subfigure}[b]{0.45\textwidth}
